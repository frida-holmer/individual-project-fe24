...
export async function fetchPCGames(): Promise<Game[]> {
    try {
        // const res = await fetch(`https://api.rawg.io/api/games?key=${process.env.NEXT_PUBLIC_API_KEY}&platforms=4&ordering=-metacritic&page_size=10`);
        if (!res.ok) {
            throw new Error(`HTTP error! Status: ${res.status}`);
        }
        const data = await res.json();
...
export async function fetchPlaystationGames(): Promise<Game[]> {
    try {
        // const res = await fetch(`https://api.rawg.io/api/games?key=${process.env.NEXT_PUBLIC_API_KEY}&platforms=187&ordering=-metacritic&page_size=10`);
        if (!res.ok) {
            throw new Error(`HTTP error! Status: ${res.status}`);
        }
        const data = await res.json();
...
export async function fetchXboxGames(): Promise<Game[]> {
    try {
        // const res = await fetch(`https://api.rawg.io/api/games?key=${process.env.NEXT_PUBLIC_API_KEY}&platforms=1&ordering=-metacritic&page_size=10`);
        if (!res.ok) {
            throw new Error(`HTTP error! Status: ${res.status}`);
        }
        const data = await res.json();
...
export async function fetchNintendoGames(): Promise<Game[]> {
    try {
        // const res = await fetch(`https://api.rawg.io/api/games?key=${process.env.NEXT_PUBLIC_API_KEY}&platforms=7&ordering=-metacritic&page_size=10`);
        if (!res.ok) {
            throw new Error(`HTTP error! Status: ${res.status}`);
        }
        const data = await res.json();
